    ...
    return (
        <div className="flex min-h-screen flex-col items-center bg-[#F4F6F9] text-[#243054] nunito">
            <header className="relative px-5 pt-5 pb-3 flex justify-center items-center gap-1 border-b-2 border-[#243054]/10 w-full max-w-3xl">
                <button type="button" aria-label="Volver" className="absolute left-4 top-1/2 -translate-y-1/2 bg-white rounded-full p-1.5 shadow-sm transition hover:bg-[#243054]/5 focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-[#243054]"
            ...
            <div className="w-full max-w-3xl p-5">
                ...
                {error ? ( <p className="mt-3 text-sm text-red-500">{error}</p>) : ( <CanchasList canchas={canchas} />)}
            </div>
        </div>
    );
}
