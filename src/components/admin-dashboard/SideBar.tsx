...
function SelectionSubButton({
  children,
  className,
  size,
  onSelect,
}: SelectionSubButtonProps) {
      ...
      <Sidebar className="bg-[#243054] text-white" collapsible="icon">
              ...
              <SidebarMenu>
                      ...
                      {predios.map((predio) => (
                        <SidebarMenuSubItem key={predio.id_predio}>
                          <SelectionSubButton
                            ...
                            className="select-none cursor-pointer text-white/75 hover:bg-white/10 hover:text-white active:bg-[#243054] active:text-white focus:bg-[#243054] focus:text-white"
                          >
                ...
                <SidebarMenuItem>
                  ...
                  >
                    ...
                    <span>Canchas</span>
                    <ChevronDown
                      className={`ml-auto transition-transform ${canchasOpen ? "" : "-rotate-90"}`}
                    />
                  </SidebarMenuButton>
                  {canchasOpen && (
                    <SidebarMenuSub>
                      {predios.map((predio) => (
                        <SidebarMenuSubItem key={predio.id_predio}>
                          <SidebarMenuSubButton className="select-none cursor-pointer text-white/75 hover:bg-white/10 hover:text-white active:bg-[#243054] active:text-white focus:bg-[#243054] focus:text-white">
                            <span>{predio.nombre}</span>
                          </SidebarMenuSubButton>
                          <SidebarMenuSub>
                            {predio.canchas.map((cancha) => (
                              <SidebarMenuSubItem key={cancha.id_cancha}>
                                <SelectionSubButton
                                  ...
                                  size="sm"
                                  className="select-none cursor-pointer text-white/60 hover:bg-white/10 hover:text-white active:bg-[#243054] active:text-white focus:bg-[#243054] focus:text-white"
                                >
        ...
        <SidebarFooter className="border-t border-white/10 bg-[#243054] p-5 nunito">
          <div className="mb-4 rounded-xl border border-dashed border-white/25 bg-white/5 p-4 group-data-[collapsible=icon]:hidden">
            <p className="text-sm font-bold text-white">¿Necesitás ayuda?</p>
